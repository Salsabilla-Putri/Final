/**
 * Gen-Track Dashboard v5.0 - Sidebar dinamis, parameter lengkap, estimasi umur, biaya, perawatan
 */
'use strict';

// ---------- Constants ----------
const VOLT_LO = 198, VOLT_HI = 242;
const FREQ_LO = 49.5, FREQ_HI = 50.5;
const ENGINE_LIFE_H = 10000;          // total umur mesin (jam)
const AVG_CONSUMPTION_LPH = 2.5;      // konsumsi BBM rata-rata (L/jam)
const FUEL_PRICE_PER_LITER = 12000;   // Rp per liter
const TANK_CAPACITY_L = 50;           // liter

// Maintenance items (interval jam)
const MAINT_ITEMS = [
    { name: 'Ganti Oli', interval: 250 },
    { name: 'Filter Udara', interval: 500 },
    { name: 'Busi', interval: 1000 },
    { name: 'Coolant', interval: 1000 },
    { name: 'Servis Besar', interval: 2500 }
];
...
let currentData = {
    volt: 0, freq: 0, current: 0, power: 0,
    rpm: 0, iat: 0, coolant: 0, fuel: 0, afr: 0, throttle: 0,
    engineHours: 0, isRunning: false, syncOk: false, source: 'OFF'
};
...
function renderRecentActivities() {
    ...
    if (eventLog.length === 0) {
        container.innerHTML = '<div class="activity-item">Tidak ada aktivitas baru</div>';
        return;
    }
    container.innerHTML = eventLog.slice(0,5).map(ev => `
        <div class="activity-item">
            <i class="fas fa-${ev.type === 'source' ? 'bolt' : 'circle-info'}"></i>
            <span>${ev.text}</span>
            <span style="margin-left:auto; font-size:0.65rem;">${ev.time}</span>
        </div>
    `).join('');
}
...
function renderRecentAlerts() {
    ...
    if (alerts.length === 0) {
        container.innerHTML = '<div class="alert-item info"><i class="fas fa-check-circle"></i> Tidak ada alert aktif</div>';
        return;
    }
    container.innerHTML = alerts.map(alert => `
        <div class="alert-item ${alert.status}">
            <i class="fas fa-exclamation-triangle"></i> ${alert.text}
            <span style="margin-left:auto; font-size:0.65rem;">${alert.time}</span>
        </div>
    `).join('');
}
...
function initCharts() {
    ...
    if (ctx && !uptimeChart) {
        uptimeChart = new Chart(ctx, {
            ...
            options: { responsive: true, maintainAspectRatio: true, scales: { y: { beginAtZero: true, max: 24 } }, plugins: { legend: { display: false } } }
        });
    }
    ...
    if (dctx && !donutChart) {
        donutChart = new Chart(dctx, {
            type: 'doughnut',
            data: { datasets: [{ data: [0, ENGINE_LIFE_H], backgroundColor: ['#1f6392', '#e6f0f9'], borderWidth: 0 }] },
            options: { cutout: '70%', responsive: false, maintainAspectRatio: true, plugins: { legend: { display: false }, tooltip: { enabled: false } } }
        });
    }
}

function updateLifespan(engineHours) {
    const used = clamp(engineHours, 0, ENGINE_LIFE_H);
    const remain = ENGINE_LIFE_H - used;
    if (donutChart) {
        donutChart.data.datasets[0].data = [used, remain];
        donutChart.update();
    }
    $('#lifespanRemainText').innerText = remain.toFixed(0) + ' jam';
    return remain;
}
...
function updateTodayActive(isRunning) {
    if (isRunning) {
        const now = Date.now();
        if (lastActiveTick) {
            const deltaSec = (now - lastActiveTick) / 1000;
    ...
    $('#engToday').innerText = `${hours}h ${mins}m`;
}
