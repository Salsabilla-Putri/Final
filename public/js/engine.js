...
async function fetchData() {
    try {
        ...
        if (json.success) {
            const t1 = Date.now(); // waktu browser terima data
            updateDashboard(json.data);

            const ts = Date.parse(json.data?.timestamp || '');
            const isFresh = Number.isFinite(ts) && (t1 - ts <= ESP_FRESHNESS_MS);
            ...
            if (Number.isFinite(ts)) {
                const latency = t1 - ts;
                console.log(
                    `[LATENCY] t0(ESP32)=${new Date(ts).toISOString()} | ` +
                    `t1(Browser)=${new Date(t1).toISOString()} | ` +
                    `Latency=${latency} ms`
                );
                // Simpan ke array untuk rekapitulasi
                if (!window._latencyLog) window._latencyLog = [];
                window._latencyLog.push(latency);
                // Tampilkan statistik setiap 20 data
                if (window._latencyLog.length % 20 === 0) {
                    const arr = window._latencyLog.slice(-20);
                    const avg = (arr.reduce((a,b)=>a+b,0)/arr.length).toFixed(0);
                    const min = Math.min(...arr);
                    const max = Math.max(...arr);
                    const std = Math.sqrt(arr.reduce((s,v)=>s+Math.pow(v-avg,2),0)/arr.length).toFixed(0);
                    console.log(
                        `%c[LATENCY REKAP - 20 data terakhir] ` +
                        `Avg=${avg}ms | Min=${min}ms | Max=${max}ms | StdDev=${std}ms`,
